  ...
  protected:
    ...
    void visit(MaybeConst<StmtAST> &node) {
        ...
        case StmtASTKind::Expr:
            ...
            break;
        case StmtASTKind::Break:
            derived().visit(static_cast<MaybeConst<BreakStmtAST> &>(node));
            break;
        ...
        case StmtASTKind::Local:
            ...
            break;
        case StmtASTKind::Assign:
            derived().visit(static_cast<MaybeConst<AssignStmtAST> &>(node));
            break;
        case StmtASTKind::Block:
            derived().visit(static_cast<MaybeConst<BlockStmtAST> &>(node));
            break;
        case StmtASTKind::If:
            derived().visit(static_cast<MaybeConst<IfStmtAST> &>(node));
            break;
        case StmtASTKind::While:
            derived().visit(static_cast<MaybeConst<WhileStmtAST> &>(node));
            break;
        }
    }
